 ...
 * 
 * <p>
 * このクラスは、特定のブロック（BlockId）に対する共有ロック（SHARED）と
 ...
 * <h3>ロックアップグレード</h3>
 * <p>
 * 共有ロックを持っているトランザクションが排他ロックを要求した場合、
 * 他に共有ロックを持つトランザクションがいなければ、アップグレードが許可されます。
 * 
 * <h3>スレッドセーフティ</h3>
 * <p>
 * このクラスはスレッドセーフです。内部で {@link ReentrantLock} を使用して
...
public class Lock {
     ...
     * 
     * <p>
     * ロックを取得できるまで待機します。タイムアウトした場合は例外をスローします。
     * 
